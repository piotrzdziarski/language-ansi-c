...
int numberAtPosition(int n, int p){int divider = 1,i,singleN;for(i=1;(n / divider)>=10;i++)divider=power(10,i);if(p>i)return 0;for(;p!=i;i--){singleN=n/divider;n-=singleN*divider;divider/=10;}return n/divider;}
...
for (int i = MAX_HIST_NUMBERS_COUNT; i > 0; i--) {
	for (int j = 0; j < MAX_HIST; j++) {
		int single_number = numberAtPosition(j + 1, i);
		if (!h[j]) continue;
